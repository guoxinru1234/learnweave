...
export function QuizView({ questions, sceneId }: QuizViewProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Persist every incorrect answer so the dedicated wrong-question book updates immediately.
      ordered.filter((r) => r.correct === false).forEach((r) => {
        const q = questions.find((item) => item.id === r.questionId);
        if (!q) return;
        void api.createNote({
          title: `错题：${q.question}`,
          content: `你的答案：${String(answers[q.id] ?? '未作答')}\n正确答案：${Array.isArray(q.answer) ? q.answer.join(', ') : String(q.answer ?? '见解析')}\n解析：${q.analysis ?? '请回顾相关知识点。'}`,
